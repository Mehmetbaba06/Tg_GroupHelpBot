...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Avengers AI Already Activated In This Chat")
            return
        await lel.edit(
            f"Avengers AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("Avengers AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Avengers AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("Daisy", "Aco")
        URL = "https://api.affiliateplus.xyz/api/chatbot?message=hi&botname=@Avengers_Helperbot&ownername=@Avengers_Player"
        ...
        test = test.replace("daisy", "Aco")
        test = test.replace("Daisy", "Aco")
        URL = f"https://api.affiliateplus.xyz/api/chatbot?message={test}&botname=Avengers-bot&ownername=@Avengers_Player"
        try:
            r = requests.request("GET", url=URL)
        except:
            return
...
async def inuka(client, message):
    ...
    test = test.replace("daisy", "Aco")
    test = test.replace("Daisy", "Aco")
    URL = f"https://api.affiliateplus.xyz/api/chatbot?message={test}&botname=Avengers-Bot&ownername=@Akcrazytech"
    try:
        r = requests.request("GET", url=URL)
    except:
        return
...
@daisyx.on_message(
    filters.regex("Avengers-Bot|Avengers-Bot|Avengers-Bot|Avengers-Bot|Avengers-Bot")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
<b> Chatbot </b>
Avengers-Bot AI IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
