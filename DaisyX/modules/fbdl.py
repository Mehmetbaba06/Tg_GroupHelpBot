...
async def _(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        kk = f"{filename}.mp4"
        caption = f"Facebook Video downloaded Successfully by @Avengers_HelperBot.\nSay hi @AkcrazytechGiveaways."

        await tbot.send_file(
            event.chat_id,
            kk,
            caption="Facebook Video downloaded Successfully by @Avengers_HelperBot.\nSay hi to @AkcrazytechGiveaways.",
        )
        os.system(f"rm {kk}")
    else:
