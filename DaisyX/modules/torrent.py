...
async def _(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Avengers-Bot found some torrents for you. Take a look 👇",
        buttons=[
            [
                Button.inline(
...
async def paginate_prevtorrent(event):
    ...
    try:
        ...
        await event.reply("Sorry, Avengers-Bot Cant found any torrents for that word")
        print(e)
        return
    vector = len(results)
    if num < 0:
...
async def torrentstop(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Thanks for using.\n❤️ from [Avengers-Bot](t.me/Avengers_HelperBot) !",
        link_preview=False,
    )
